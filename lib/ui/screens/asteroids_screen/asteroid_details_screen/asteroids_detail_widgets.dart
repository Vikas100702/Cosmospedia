          ...
          children: [
            ...
            SizedBox(
              ...
              child: CustomPaint(
                painter: OrbitPainter(
                  ...
                  closeApproachData: asteroid.closeApproachData.isNotEmpty
                      ? asteroid.closeApproachData.first
                      : null,
                ),
              ),
            ...
            Wrap(
              ...
              children: [
                ...
                if (isHazardous)
                  _buildLegendItem(Colors.orange, 'Intersection Points'),
                _buildLegendItem(Colors.yellow, 'Sun'),
              ],
            ),
            const SizedBox(height: 16),
            Container(
...
class OrbitPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final earthOrbitRadius = size.width * 0.35;
    ...
    final animationValue = closeApproachData != null
    ...
    // Draw orbits first
    _drawOrbits(canvas, center, earthOrbitRadius,
        asteroidOrbitRadiusX, asteroidOrbitRadiusY, asteroidOrbitAngle);

    // Calculate positions
    final earthPos = _calculateEarthPosition(center, earthOrbitRadius, animationValue);
    final asteroidPos = _calculateAsteroidPosition(
        center, asteroidOrbitRadiusX, asteroidOrbitRadiusY,
        asteroidOrbitAngle, animationValue);
    ...
    if (isHazardous) {
      _drawIntersectionPoints(
          canvas, center, earthOrbitRadius,
          asteroidOrbitRadiusX, asteroidOrbitRadiusY, asteroidOrbitAngle);
    }

    // Draw earth and asteroid
    _drawEarth(canvas, earthPos);
    _drawAsteroid(canvas, asteroidPos);

    // Draw labels last so they're on top
    _drawLabels(canvas, center, earthPos, asteroidPos);
  }
  ...
  void _drawOrbits(
      Canvas canvas,
      Offset center,
      double earthOrbitRadius,
      double asteroidOrbitRadiusX,
      double asteroidOrbitRadiusY,
      double asteroidOrbitAngle,
      ) {
    // Earth's orbit
    final earthOrbitPaint = Paint()
      ...
      ).createShader(Rect.fromCircle(center: center, radius: earthOrbitRadius));
    ...
    final asteroidOrbitPaint = Paint()
      ...
      ..shader = SweepGradient(
        colors: [
          isHazardous ? Colors.red.withOpacity(0.8) : Colors.green.withOpacity(0.8),
          isHazardous ? Colors.red.withOpacity(0.1) : Colors.green.withOpacity(0.1),
        ],
        stops: [0.0, 1.0],
        startAngle: 0,
        endAngle: math.pi * 2,
  ...
  void _drawSun(Canvas canvas, Offset center) {
    ...
    // Draw sun surface with some texture
    canvas.drawCircle(center, 20, sunPaint);

    // Add some sun surface details
    final sunDetailPaint = Paint()
    ...
    for (int i = 0; i < 8; i++) {
      ...
      canvas.drawLine(center, offset, sunDetailPaint);
    }
  }

  Offset _calculateEarthPosition(Offset center, double earthOrbitRadius, double animationValue) {
    final earthAngle = 2 * math.pi * animationValue;
    return Offset(
      center.dx + earthOrbitRadius * math.cos(earthAngle),
      center.dy + earthOrbitRadius * math.sin(earthAngle),
    );
  }

  Offset _calculateAsteroidPosition(
      Offset center,
      double asteroidOrbitRadiusX,
      double asteroidOrbitRadiusY,
      double asteroidOrbitAngle,
    ...
    final asteroidAngle = 2 * math.pi * (animationValue + 0.3);
    return Offset(
      center.dx + asteroidOrbitRadiusX * math.cos(asteroidAngle + asteroidOrbitAngle),
      center.dy + asteroidOrbitRadiusY * math.sin(asteroidAngle + asteroidOrbitAngle),
    );
  }

  void _drawEarth(Canvas canvas, Offset position) {
    // Earth glow
    final earthGlowPaint = Paint()
      ..shader = RadialGradient(
        colors: [
          Colors.blue.withOpacity(0.5),
          Colors.blue.withOpacity(0.0),
    ...
    // Draw earth with glow
    canvas.drawCircle(position, 15, earthGlowPaint);
    canvas.drawCircle(position, 10, earthPaint);

    // Add some earth details
    final earthDetailPaint = Paint()
      ..color = Colors.green.withOpacity(0.7)
      ..style = PaintingStyle.fill;

    // Draw some random continents
    canvas.drawCircle(
    ...
    canvas.drawCircle(
      Offset(position.dx + 4, position.dy + 3),
      2,
      earthDetailPaint,
    );
  }

  void _drawAsteroid(Canvas canvas, Offset position) {
    // Asteroid glow
    final asteroidGlowPaint = Paint()
      ..shader = RadialGradient(
        colors: [
          isHazardous ? Colors.red.withOpacity(0.5) : Colors.green.withOpacity(0.5),
          Colors.transparent,
        ],
        stops: [0.0, 1.0],
    ...
    final asteroidPaint = Paint()
      ..color = isHazardous ? Colors.red.shade700 : Colors.grey.shade600;

    // Draw asteroid with glow
    canvas.drawCircle(position, 12, asteroidGlowPaint);

    // Draw irregular asteroid shape
    final asteroidPath = Path();
    asteroidPath.addOval(Rect.fromCircle(center: position, radius: 8));

    // Make it irregular
    final perturbationPaint = Paint()
      ..color = isHazardous ? Colors.red.shade900 : Colors.grey.shade800
      ..style = PaintingStyle.fill;

    // Add some craters
    for (int i = 0; i < 5; i++) {
      final angle = random.nextDouble() * math.pi * 2;
      final distance = random.nextDouble() * 5;
      final craterPos = Offset(
        position.dx + math.cos(angle) * distance,
        position.dy + math.sin(angle) * distance,
      );
      canvas.drawCircle(craterPos, random.nextDouble() * 2 + 1, perturbationPaint);
    }
  ...
  void _drawIntersectionPoints(
    ...
    final intersectionPoint1 = _calculateIntersectionPoint(
        center, earthOrbitRadius, asteroidOrbitRadiusX,
        asteroidOrbitRadiusY, asteroidOrbitAngle, 1);

    final intersectionPoint2 = _calculateIntersectionPoint(
        center, earthOrbitRadius, asteroidOrbitRadiusX,
        asteroidOrbitRadiusY, asteroidOrbitAngle, 2);

    // Draw intersection points with pulsing effect
    final time = DateTime.now().millisecondsSinceEpoch % 2000 / 2000;
    final pulseFactor = (sin(time * math.pi * 2) * 0.2 + 1.0);

    canvas.drawCircle(
      intersectionPoint1,
      6 * pulseFactor,
      intersectionPaint..color = Colors.orange.withOpacity(0.8),
    );
    ...
    const double warningSize = 10;

    // First intersection warning
    canvas.save();
  ...
  void _drawTextWithBackground(
    ...
    textPainter.paint(
      ...
      Offset(
        ...
        position.dy + offset.dy - textPainter.height / 2,
      ),
    );
  }
  ...
  @override
  bool shouldRepaint(covariant CustomPainter oldDelegate) => true; // Animate continuously
}
